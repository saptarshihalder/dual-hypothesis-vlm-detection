...
OUTPUT_DIR = "/NAS_DISK/Saptarshi_data/pipeline_output/semantic_grok"
...
class SemanticDataset(Dataset):
    def __init__(self, paths_and_ids, prototypes, preprocess, augment=True):
...
def main():
    ...
    log.info(f"Contrastive: {LAMBDA_CONTRAST}, CE: {LAMBDA_CE}, Temp: {TEMPERATURE}")
    log.info("=" * 70)
    ...
    log.info("  Student: CLIP encoder (frozen) + projector + classifier")
    ...
    log.info(f"\n  Training with prototypes: {len(coco_tr_p)} real + {len(mj_tr_p)} fake")

    if len(coco_tr_p) == 0 or len(mj_tr_p) == 0:
        log.error("No training images matched prototypes!")
        log.info(f"  Proto IDs sample: {list(prototypes.keys())[:5]}")
        log.info(f"  COCO IDs sample: {[i for _, i in coco_tr[:5]]}")
        log.info(f"  MJ IDs sample: {[i for _, i in mj_tr[:5]]}")
        sys.exit(1)

    train_ds = ConcatDataset([
        SemanticDataset(coco_tr_p, prototypes, preprocess, augment=True),
        SemanticDataset(mj_tr_p, prototypes, preprocess, augment=True),
    ])
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        for imgs, labels, e_real, e_fake in tqdm(train_loader, desc=f"Ep {epoch+1}/{MAX_EPOCHS}", leave=False):
            imgs = imgs.cuda()
        ...
        if (epoch + 1) % EVAL_EVERY == 0 or epoch == 0:
            epoch_res = {"epoch": epoch + 1, "loss": round(total_loss / n_b, 4),
                         "contrastive": round(total_con / n_b, 4),
                         "ce": round(total_ce / n_b, 4),
    ...
    log.info(f"\n  SEMANTIC TRANSFER: VLM cue embeddings → student alignment")
    log.info(f"  Previous student used only P(fake) soft labels")
    log.info(f"  This student aligns with actual text semantics of reasoning")
